...
static _cmx_request *nb_state = NULL;/* keep track of all nonblocking operations */
...
/* worker functions */
STATIC void nb_send_common(void *buf, int count, int dest, _cmx_request *nb,
    int need_free);
...
STATIC void nb_recv_packed(void *buf, int count, int source, _cmx_request *nb,
    stride_t *stride);
STATIC void nb_recv_datatype(void *buf, MPI_Datatype dt, int source, _cmx_request *nb);
STATIC void nb_recv_iov(void *buf, int count, int source, _cmx_request *nb,
    _cmx_giov_t *iov);
...
int cmx_init()
{
    ...
    {
        char *value = NULL;
        nb_max_outstanding = CMX_MAX_NB_OUTSTANDING; /* default */
    ...
    nb_state = (_cmx_request*)malloc(sizeof(_cmx_request) * nb_max_outstanding);
    CMX_ASSERT(nb_state);
        ...
        int proc, cmx_handle_t cmx_hdl)
{
    ...
    void *dst = find_alloc_ptr(cmx_hdl, proc);
        ...
        int proc, cmx_handle_t cmx_hdl)
{
    ...
    if (op == CMX_ACC_INT) {
      ...
      datatype = CMX_COMPLEX;
    } else if (op == CMX_ACC_DCP) {
      datatype = CMX_DCMPLX;
    } else {
      cmx_error("cmx_acc: unknown datatype in accumulate", -1);
    }
        ...
        int proc, cmx_handle_t cmx_hdl)
{
    ...
    if (op == CMX_ACC_INT) {
      ...
      cmx_error("cmx_accs: unknown datatype in accumulate", -1);
    }
        ...
        int proc, cmx_handle_t cmx_hdl)
{
     ...
     * implementation */
    cmxInt count = iov->count;
    int i;
    tiov.src = malloc(count*sizeof(void*));
    tiov.dst = malloc(count*sizeof(void*));
    tiov.count = count;
    tiov.bytes = iov->bytes; 

    for (i=0; i<count; i++) {
      tiov.src[i] = iov->loc[i];
      tiov.dst[i] = (void*)((char*)dst+iov->rem[i]);
    }
        ...
        int proc, cmx_handle_t cmx_hdl)
{
    ...
    for (i=0; i<count; i++) {
      tiov.src[i] = (void*)((char*)src+iov->rem[i]);
      tiov.dst[i] = iov->loc[i];
    }
        ...
        int proc, cmx_handle_t cmx_hdl)
{
    ...
    if (op == CMX_ACC_INT) {
      ...
      cmx_error("cmx_accv: unknown datatype in accumulate", -1);
    }
...
int cmx_fence_all(cmx_igroup_t *igroup)
{
    int p = 0;
    int count_before = 0;
    int count_after = 0;
    ...
    /* count how many fence messages to send */
...
int cmx_fence_proc(int proc, cmx_igroup_t *group)
{
    int world_rank = -1;
    int master_rank = -1;
    cmx_igroup_t *igroup = NULL;
...
STATIC reg_entry_t* _cmx_malloc_local(size_t size)
{
    ...
    reg_entry = reg_cache_insert(
            g_state.rank, CMX_GROUP_WORLD->rank, memory, size, name, memory);
    if (NULL == reg_entry) {
...
int cmx_wait(cmx_request_t* hdl)
{
    int index = 0;
    _cmx_request *nb = NULL;
    ...
    for (index=0; index<nb_max_outstanding; index++) {
      if (hdl == &nb_state[index]) {
        nb = &nb_state[index];
        break;
      }
    }
    ...
    if (0 == nb->in_use) {
        fprintf(stderr, "{%d} cmx_wait Error: invalid handle\n",
                g_state.rank);
    }
...
int cmx_test(cmx_request_t* hdl, int *status)
{
    int index = 0;
    nb_t *nb = NULL;
    ...
    if (NULL == nb->send_head && NULL == nb->recv_head) {
        CMX_ASSERT(0 == nb->send_size);
        CMX_ASSERT(0 == nb->recv_size);
        *status = 0;
        nb->in_use = 0;
    }
    else {
        ...
        cmx_request_t **hdl)
{
    ...
    if (op == CMX_ACC_INT) {
      ...
      cmx_error("cmx_nbacc: unknown datatype in accumulate", -1);
    }
        ...
        cmx_request_t **hdl)
{
    ...
    if (op == CMX_ACC_INT) {
      ...
      cmx_error("cmx_nbaccs: unknown datatype in accumulate", -1);
    }
        ...
        cmx_request_t **hdl)
{
    ...
    cmxInt count = iov->count;
    int i;
    printf("p[%d] (cmx_nb_putv) Got to 1\n",g_state.rank);
    tiov.src = malloc(count*sizeof(void*));
    tiov.dst = malloc(count*sizeof(void*));
    tiov.count = count;
    tiov.bytes = iov->bytes;
    printf("p[%d] (cmx_nb_putv) Got to 2\n",g_state.rank);

    for (i=0; i<count; i++) {
      tiov.src[i] = iov->loc[i];
      tiov.dst[i] = (void*)((char*)dst+iov->rem[i]);
    }
    printf("p[%d] (cmx_nb_putv) Got to 3\n",g_state.rank);
    ...
    printf("p[%d] (cmx_nb_putv) Got to 4\n",g_state.rank);

    igroup = cmx_hdl.group;
    CHECK_GROUP(igroup,proc);
    world_proc = _get_world_rank(igroup, proc);
    printf("p[%d] (cmx_nb_putv) Got to 5 iov_len: %d world_proc: %d\n",g_state.rank,
        iov_len,world_proc);

    nb_putv(&tiov, iov_len, world_proc, nb);
    printf("p[%d] (cmx_nb_putv) Got to 6\n",g_state.rank);

    free(tiov.src);
    free(tiov.dst);
    printf("p[%d] (cmx_nb_putv) Got to 7\n",g_state.rank);
        ...
        cmx_request_t **hdl)
{
    ...
    for (i=0; i<count; i++) {
      tiov.dst[i] = (void*)((char*)src+iov->rem[i]);
      tiov.src[i] = iov->loc[i];
    }
        ...
        cmx_request_t **hdl)
{
    ...
    if (op == CMX_ACC_INT) {
      ...
      cmx_error("cmx_nbaccv: unknown datatype in accumulate", -1);
    }
        ...
        int proc, cmx_handle_t cmx_hdl)
{
    ...
    cmx_igroup_t *igroup = NULL;
    nb_t *nb = NULL;
    void *prem = find_alloc_ptr(cmx_hdl, proc);
...
int cmx_malloc(cmx_handle_t *handle, cmxInt size, cmx_igroup_t *igroup)
{
    ...
    for (i=0; i<igroup->size; ++i) {
      ...
      link->rank = reg_entries[i].wrank;
      link->next = prev;
      prev = link;
    }
    handle->group = igroup;
    handle->rank = igroup->rank;
    handle->buf = my_reg.mapped;
    handle->bytes = my_reg.len;
...
int cmx_free(cmx_handle_t handle)
{
    ...
    CMX_ASSERT(ptrs);
    ptrs[igroup->rank] = find_alloc_ptr(handle, igroup->rank);
...
STATIC void nb_recv_iov(void *buf, int count, int source, nb_t *nb, _cmx_giov_t *iov)
{
    int retval = 0;
    message_t *message = NULL;
...
STATIC void nb_wait_all()
{
    ...
    CMX_ASSERT(nb_count_event-nb_count_event_processed >= 0);

    for (i=0; i<nb_max_outstanding; ++i) {
        nb_wait_for_all(&nb_state[i]);
        CMX_ASSERT(nb_state[i].send_size == 0);
        CMX_ASSERT(nb_state[i].send_head == NULL);
        CMX_ASSERT(nb_state[i].send_tail == NULL);
        CMX_ASSERT(nb_state[i].recv_size == 0);
        CMX_ASSERT(nb_state[i].recv_head == NULL);
        CMX_ASSERT(nb_state[i].recv_tail == NULL);
    }
    CMX_ASSERT(nb_count_event == nb_count_event_processed);
    CMX_ASSERT(nb_count_send == nb_count_send_processed);
    CMX_ASSERT(nb_count_recv == nb_count_recv_processed);
}
...
STATIC void nb_putv_packed(_cmx_giov_t *iov, int proc, nb_t *nb)
{
    int i = 0;
    void **src = NULL;
    void **dst = NULL;
...
STATIC void nb_getv_packed(_cmx_giov_t *iov, int proc, nb_t *nb)
{
    void **src = NULL;
    void **dst = NULL;
    int bytes = 0;
