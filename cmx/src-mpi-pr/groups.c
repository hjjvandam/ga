...
/* world group state. Contains communicator for all processors, including
 * progress ranks */
...
static void _create_group(cmx_igroup_t **igroup);
static void _igroup_free(cmx_igroup_t *igroup);
static long xgethostid();


/**
 * Creates and associates a cmx igroup.
 *
...
int cmx_group_create(
        int n, int *pid_list, cmx_igroup_t *parent, cmx_igroup_t **child)
{
    int status = 0;
    int grp_me = 0;
...
#endif
        /* world group is the first group in linked list */
